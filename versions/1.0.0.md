...
Describes the steps to be taken across one or more APIs to achieve an objective. The procedure option MAY define variables needed in order to execute procedural steps, where the defined steps represent a call to an API operation or another procedure, and a set of outputs.
...
<a name="procedureSteps"></a>steps | [[Step Object](#stepObject)] | An ordered list of steps where each step represents a call to an API operation or to another procedure
<a name="outputs"></a>outputs | Map[`string`, ???] | Specifies the values that are returned from a procedure.
...
<a name="stepId"></a>stepId | `string` | Unique string to represent the step. The id MUST be unique amongst all steps describe in the workflow. The stepId value is **case-sensitive**. Tools and libraries MAY use the stepId to uniquely identify a procedure step, therefore, it is RECOMMENDED to follow common programming naming conventions.
<a name="stepOperationId"></a>operationId | `string` | TBD 
<a name="stepOperationRef"></a>operationRef | `string` | TBD
<a name="stepParameters"></a>parameters | [[Parameter Object](#parameterObject)] | A list of parameters that are applicable for this step. The list MUST NOT include duplicated parameters. 
...
<a name="stepFailure"></a>failure | [???] | TBD
