...
| <a name="stepOperationId"></a>operationId         |                                         `string`                                         | The name of an existing, resolvable operation, as defined with a unique `operationId` and existing within one of the `sourceDescriptions`. The referenced operation will be invoked by this workflow step. If multiple (non `arazzo` type) `sourceDescriptions` are defined, then the `operationId` MUST be specified using a [Runtime Expression](#runtime-expressions) (e.g., `$sourceDescriptions.<name>.<operationId>`) to avoid ambiguity or potential clashes. This field is mutually exclusive of the `operationPath`, `operationName`, and `workflowId` fields respectively. This field MUST NOT be used to reference a `wsdl` type source description.                                                                                                                                                                                                                                                                                          |
| <a name="stepOperationName"></a>operationName     |                                         `string`                                         | The name of an existing, resolvable WSDL operation, as defined within a `wsdl` type source description. The value MUST match the `name` attribute of a `wsdl:operation` element within the WSDL document's `portType` (WSDL 1.1) or `interface` (WSDL 2.0) definition. If multiple `wsdl` type `sourceDescriptions` are defined, then the `operationName` MUST be specified using a [Runtime Expression](#runtime-expressions) (e.g., `$sourceDescriptions.<name>.<operationName>`) to avoid ambiguity or potential clashes. This field is mutually exclusive of the `operationId`, `operationPath`, `channelPath`, and `workflowId` fields respectively.                                                                                                                                                                                                                                                                                                |
...
For steps that reference a `wsdl` type source description via `operationName`, tools MUST resolve the HTTP method and endpoint URL from the WSDL binding definition. Workflow authors do not specify these directly.
