...
A `protobuf` Source Description identifies a Protocol Buffers source document. When referenced by `grpcMethod`, service and RPC declarations in that source are interpreted using the gRPC execution semantics defined by this specification. A Protocol Buffer source description MAY contain multiple service declarations. Tooling MUST resolve imports and dependencies as part of source resolution.

Arazzo does not define:

- protobuf import roots; or
- gRPC deployment targets or discovery mechanisms; or
- channel construction, reflection, or generated clients.

These are implementation concerns. See the [Protocol Buffers language guide](https://protobuf.dev/programming-guides/proto3/) and [gRPC core concepts](https://grpc.io/docs/what-is-grpc/core-concepts/) for the underlying message and service model.
...
| <a name="stepGrpcMethod"></a>grpcMethod           |                                         `string`                                         | A fully-qualified gRPC method identifier from a Protocol Buffer service declaration, in the form `$sourceDescriptions.<name>.<fully-qualified-service-name>/<method-name>`; the logical identifier is `<fully-qualified-service-name>/<method-name>` (e.g., `org.v1.Library/GetBook`). This field is mutually exclusive with `operationId`, `operationPath`, `channelPath`, and `workflowId` and only applicable for `grpc` steps.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
##### gRPC Step Semantics for Protocol Buffer Source Descriptions

###### Method Identity and Message Representation

For a gRPC Step, the referenced Protocol Buffer service declaration determines the request and response message types and the RPC's streaming form. `operationId`, `operationPath`, and `channelPath` MUST NOT identify a gRPC method. When Protocol Buffer messages are represented as Arazzo values, implementations MUST use the [ProtoJSON representation](https://protobuf.dev/programming-guides/json/) for the declared type, including its rules for nested messages, repeated fields, maps, enums, bytes, and field naming; Arazzo does not model the Protocol Buffers binary wire format.

The logical gRPC method identity is `<fully-qualified-service-name>/<method-name>`. For example:
...
identifies the gRPC method `org.v1.Library/GetBook`. A source-qualified reference is written as:
...
```text
$sourceDescriptions.grpcLibrary. | org.v1.Library/GetBook
        Arazzo source           gRPC method identifier
```
...
For client-streaming RPCs, a Request Body Object represents one streamed request message followed by a client request-direction half-close. An ordered array of Request Body Objects sends those messages in authored order and then half-closes the client request direction. An empty array sends zero outbound messages followed by immediate half-close. If `requestBody` is omitted, the client sends zero outbound messages followed by immediate half-close.

Bidirectional streaming uses the same outbound request representation as client streaming. Outbound messages are submitted in authored order. Server responses MAY arrive while client messages are still being sent; the two directions are independent. After all configured outbound messages are successfully submitted, the client request direction is half-closed, and receiving continues until terminal gRPC status. If `requestBody` is omitted, the same zero-message and immediate half-close semantics as client streaming apply. There is no Arazzo primitive for making a later outbound message depend on an earlier inbound message: response-dependent or interactive bidi sequencing is not modeled. Ordinary bidirectional streaming remains supported.
...
###### Completion, Status, and Timeouts

A gRPC Step completes when terminal [gRPC status](https://grpc.io/docs/guides/status-codes/) is known. A gRPC call MAY terminate with status and no response Protocol Buffer message, including trailers-only or error termination. For server-streaming and bidirectional streaming, all response messages received before terminal status are retained in receive order as `$response.body`; the array MAY be empty. For unary and client-streaming RPCs, `$response.body` is the single received response message; if no response message was received, the body is absent or unavailable. Implementations MUST NOT synthesize a Protocol Buffer default response object.
...
- `details` contains structured rich error details when available, represented using ProtoJSON, and is an empty array when no details are available. When Protocol Buffer `Any` values are represented, they use ProtoJSON's normal `@type` representation. Servers are not required to provide rich status details.
...
This `successCriteria` MAY intentionally treat `NOT_FOUND` as an expected workflow outcome. If `successCriteria` is omitted, gRPC status `OK` (`code == 0`) results in Step success and a non-OK gRPC status results in Step failure. `$statusCode` retains its existing transport/HTTP meaning; `$response.status` is the terminal gRPC application status.

Server-streaming and bidirectional streaming Steps complete normally only when terminal gRPC status is received. This proposal does not model receiving from an intentionally infinite stream, stopping after a matching condition, successfully cancelling the stream, and continuing the workflow. The existing `timeout` remains a failure bound.

The Step `timeout` bounds the entire gRPC RPC, including sending client-stream messages, receiving server-stream messages, and waiting for terminal status. On expiry, the implementation MUST cancel or abort the RPC and fail the Step; normal `onFailure` semantics apply. Implementations SHOULD map this behavior to an appropriate gRPC deadline or cancellation mechanism.

###### Payload Representation and Source-Aware Validation

For gRPC, `contentType` SHOULD normally be omitted because the referenced RPC declaration determines the message type and gRPC serialization; the Arazzo payload represents the logical Protocol Buffer message using ProtoJSON.
...
unless an explicit protobuf `json_name` overrides the field name. Runtime expressions therefore use `$response.body#/bookId` and `$response.body#/chunksReceived`. See the [ProtoJSON specification](https://protobuf.dev/programming-guides/json/) for detailed conversion semantics.

JSON Schema validates the gRPC Step shape, but it cannot load a referenced `.proto`. Source-aware validation MUST verify that a source-qualified `grpcMethod` resolves to a Protocol Buffer source and to an existing service and RPC declaration, and MUST validate the payload and request-body shape against that declaration.
...
A single request body or message content object describing the `Content-Type` and request body or message content to be passed by a step to an operation or gRPC method.
...
- For a `grpcMethod` Step, when the payload is a Protocol Buffer message represented using ProtoJSON, `target` MUST be interpreted as a [JSON Pointer](https://tools.ietf.org/html/rfc6901).
...
```abnf
  ...
  metadata-name = 1*( %x61-7A / DIGIT / "-" / "_" / "." )
      ; Canonical lowercase gRPC metadata names; names are case-insensitive
  ...
  source-reference-id = 1*CHAR
      ...
      ; gRPC method identifiers use <fully-qualified-service-name>/<method-name>
...
| Request metadata             | `$request.metadata.authorization`                                    | gRPC metadata sent with the request. Metadata names are case-insensitive.                                                                                                                        |
| Response header              | `$response.header.Server`                                            | Single header values only are available.                                                                                                                                                         |
| gRPC status                  | `$response.status#/code`                                             | The terminal gRPC status code; `0` means `OK`.                                                                                                                                                   |
| Response metadata            | `$response.metadata.x-request-id`                                    | Initial response metadata from a gRPC RPC.                                                                                                                                                       |
| Trailing metadata            | `$response.trailingMetadata.trace-id`                                | Metadata associated with gRPC RPC completion.                                                                                                                                                    |
...
| Source description reference | `$sourceDescriptions.petstore.getPetById`                            | References an operationId, gRPC method identifier, or workflowId from the named source description. Resolution priority: (1) operationId/grpcMethod/workflowId, (2) field names.                 |
| Source description field     | `$sourceDescriptions.petstore.url`                                   | References a field from the Source Description Object. Resolved when no matching operationId/grpcMethod/workflowId is found.                                                                     |
...
For gRPC Steps using a Protocol Buffer Source Description, `$request.metadata.<name>` is metadata sent with the gRPC request, `$response.status` is the terminal [gRPC status](https://grpc.io/docs/guides/status-codes/) object (which may be selected with a JSON Pointer), `$response.metadata.<name>` is initial response metadata, and `$response.trailingMetadata.<name>` is metadata associated with RPC termination.
...
- **grpcMethod** - If the referenced source description is a Protocol Buffer description, `<reference>` is matched against the fully-qualified RPC identifier `<fully-qualified-service-name>/<method-name>`.
- **Source description field** - If no operationId/grpcMethod/workflowId match is found, `<reference>` is matched against field names of the Source Description Object (e.g., `url`,
  `type`).
